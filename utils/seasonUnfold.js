...
function seasonUnfold(squares, money, coin, tree, beach, ocean, wall, gridWidth, gridHeight, date, resources, attackBtn) {
    ...
    if (Math.random() <= dangerIndex / 5)
        isHurricaneYear = true;
    if (isHurricaneYear) {
    ...
    for (let i = 0; i < attackSelection.length; i++) {
        ...
        if ((id >= 0 && id < gridWidth)) {
            if (!squares[id + 1].classList.contains('ocean'))
                attackPossibilities.push(squares[id + 1]);
            if (!squares[id + gridWidth].classList.contains('ocean'))
                attackPossibilities.push(squares[id + gridWidth]);
        }
        //if bottom row
        ...
        if (attackPossibilities.length === 0)
            continue;
...
//# sourceMappingURL=seasonUnfold.js.map